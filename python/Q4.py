...
def filter_employees(employees, min_salary):
    """
    Filters & displays employees with salary greater than or equal to min_salary.
    Outputs results in a neatly formatted table, sorted by salary in descnding order.

    :param employees: List of tuples containing (name, job, salary).
    :param min_salary: Integer, minimum salary threshold.
    """
    # Filter employees based on salary &sort by salary in descnding order
...
def main():
    # Step 1: Ask  user for a file name &attempt to open  file
    ...
    try:
        with open(file_name, "r") as file:
            ...
            for line in file:
                ...
                employees.append((name, job, int(salary)))  # Convrt salary to int
    ...
    # Step 3: Enter  loop to filter employees according to salary
    while True:
        try:
            ...
            print("Invalid input. Enter a valid integer for salary.")
